...
typemenu = 'document'
img = setting.img
baper = 'SUB YT RIBASYT'
apiku = 'https://api-ribasyt.herokuapp.com/api'
gc_wa_lu = 'https://chat.whatsapp.com/GVtGl7LIA4jHz0JPyqwxse' //klo gk punya gc wa gk usah di ganti 👍
...
await limitAdd(sender, limit)
		...
		if (!isQuotedSticker) return reply('```Reply stc nya```')
		            ...
		            anu = args.join(' ').split('|')
		            satu = anu[0] !== '' ? anu[0] : `BY`
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `R I B A S`
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, jgn lupa Support YT : RibasYT```')
					fs.unlinkSync(ran)
					})
					await limitAdd(sender, limit)
					break   
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by RibasMods 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "YT : RibasYT", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
